...
{
  n <- length(xx)
  xbar <- mean(xx) # weighted mean of xx
  ybar <- mean(yy) # weighted mean of yy
  ...
  
{
  ...
  if (w==FALSE){  ## if all the weights are different from 1
    res0 <- deming(xx, yy, vr) 
  }
    
  if (w==TRUE){  ## if all the weights are different from 1
  ...
  if(boots){
      ...
      if (w==FALSE)    {results <- boot(data=dfr, statistic=newpar, R=N, vr=vr) }
      if (w==TRUE)     {results <- boot(data=dfr, statistic=newpar2, R=N, vr=vr) }
    ...
    if (keep.boot) {
      bootst <- results$t
      colnames(bootst) <- rownames(md2)
      list(results=md2, bootstraps=bootst)
    }else{
    return(md2)
  }
  }
...
cifn <- function(x,y, vr=1, w=FALSE){
  m <- ModDeming(x, y, vr=vr, w=w, boots=TRUE, keep.boot=TRUE)
  xn <- length(x)
  rx <- range(c(x, y), na.rm = TRUE)
  xd <- seq(rx[1], rx[2], length.out = xn)
  xd <- union(xd, rx)
  x.levels <- xd[order(xd)]
  nsamples <- nrow(m[[2]])
  ylevels <- matrix(nrow = nsamples, ncol = length(x.levels))
  colnames(ylevels) <- paste("X", 1:length(x.levels), sep = "")
  b0 <-  m[[1]][1,1]; b1 <- m[[1]][2,1]
  B0 <- m[[2]][,1]; B1 <- m[[2]][,2]
  for (i in seq(along = x.levels)) ylevels[, i] <- B0 + B1 * x.levels[i]
  alpha=0.05
  yy <- b0 + b1 * x.levels
  lcl <- apply(ylevels, 2, quantile, probs = alpha/2, type = 3)
  ucl <- apply(ylevels, 2, quantile, probs = 1 - alpha/2, type = 3)
  return(cbind(x.levels, yy=yy, lcl=lcl, ucl=ucl))
}      